...
### Outgoing GET_ requests must use a genuinely empty params object
`model.Js8callEvent`'s `Params` field is a fixed struct with no `omitempty` tags, so marshaling `model.Js8callEvent{Type: t}` always sends all ~25 fields at their zero value — `{"params":{"_ID":null,"DIAL":0,...}}` — instead of the documented `{"params":{}}`. Confirmed live: `RIG.GET_FREQ`/`MODE.GET_SPEED` tolerate this bloated payload fine (eventually), but `RX.GET_CALL_ACTIVITY`/`RX.GET_BAND_ACTIVITY` silently ignore it every time — no error, just no reply — while responding instantly to a genuinely empty `{"params":{}}` sent directly. `js8call.go`'s `marshalNoParamsRequest`/`noParamsRequest` produce the minimal form and are used for every argument-less outgoing request (`sendConnectEvents`, `retryUnansweredConnectRequests`); don't switch these back to `json.Marshal(model.Js8callEvent{Type: t})`.

### RIG.GET_FREQ / MODE.GET_SPEED / RX.GET_CALL_ACTIVITY / RX.GET_BAND_ACTIVITY are not answered reliably on connect
Observed directly: on a fresh connect, JS8Call reliably answers `INBOX.GET_MESSAGES`, but sometimes silently ignores the other GET_ requests sent alongside it in `sendConnectEvents` — no error, just no reply, leaving `rigStatusCache`/`callActivityCache`/`bandActivityCache` empty until something else populates them. Also observed: JS8Call's TCP server has a connection limit and returns `{"type":"API.ERROR","value":"Connections Full"}` for new connections once it's hit — heavy reconnect churn (e.g. repeatedly restarting js8web against a live JS8Call during dev) can leave stale connections that make this worse. `retryUnansweredConnectRequests` (`js8call.go`) re-sends each of the four every 4s (up to 5 times) until its cache is populated, which reliably recovers it in practice. If you see blank rig status or empty Calls/Band tabs that never resolve even after the retries exhaust, suspect JS8Call's own connection pool being clogged and restart JS8Call itself.
...
JS8Call also mixes a stray numeric `"_ID"` key directly into that same dict alongside the real per-callsign/per-offset entries (not shown in the documented example payload) — e.g. `{"K4EXA":{...},"_ID":284216288043}`. A plain `json.Unmarshal` into `map[string]CallActivityEntry` fails the whole event on this (`cannot unmarshal number into Go value of type model.CallActivityEntry`), which silently drops every call/band activity update — confirmed live as the reason the Calls/Band tabs stayed empty. `unmarshalActivityDict` (`model/js8callEvent.go`) decodes entry-by-entry via `map[string]json.RawMessage` and skips any key that doesn't parse as the expected struct, so `_ID` (and any other future unexpected key) is dropped instead of failing the batch.
